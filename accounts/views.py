...
# ---------------------------------------------------------------------------
# Portal user invite — consume the token from the invite email and let the
# user set their first password.
# ---------------------------------------------------------------------------

def portal_set_password(request, uidb64, token):
    """
    Token-protected page that lets an invited portal user set their
    initial password. Generated by `psa.views.portal_invite`. Valid for
    Django's PasswordResetTokenGenerator default lifetime (3 days).
    """
    from django.contrib.auth.models import User
    ...
    try:
        ...
        return render(request, 'accounts/portal_set_password.html',
                      {'invalid': True})

    if not default_token_generator.check_token(user, token):
        return render(request, 'accounts/portal_set_password.html',
                      {'invalid': True, 'user_email': user.email})

    if request.method == 'POST':
        pw1 = request.POST.get('password1') or ''
        pw2 = request.POST.get('password2') or ''
        if not pw1 or pw1 != pw2:
            messages.error(request, 'Passwords must match and be non-empty.')
        elif len(pw1) < 12:
            ...
            user.is_active = True
            user.save()
            # Sign the user in immediately so they land on the portal
